...
const BUSMAPS_API_URL = "https://capi.busmaps.com:8443/getGtfsFeedsDownloads";
...
export async function fetchGtfsUrl(
  apiKey: string,
  requestFn: (
    url: string,
    headers: Record<string, string>,
  ...
  const normalizedKey = apiKey.startsWith("Bearer ")
    ? apiKey
    : `Bearer ${apiKey}`;
  const response = await requestFn(BUSMAPS_API_URL, {
    "capi-key": normalizedKey,
