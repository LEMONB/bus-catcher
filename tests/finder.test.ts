...
import type { Stop, Point } from "../js/utils/time";
...
const mockRoutes: Route[] = [
  {
    route_id: "route_525",
    route_short_name: "525",
    route_long_name: "Маршрут 525",
  },
  {
    route_id: "route_32",
    route_short_name: "32",
    route_long_name: "Маршрут 32",
  },
];
...
describe("findBuses", () => {
  ...
  test("sorts buses by wait time ascending", () => {
    ...
    for (let i = 1; i < buses.length; i++) {
      expect(buses[i].waitTimeMinutes).toBeGreaterThanOrEqual(
        buses[i - 1].waitTimeMinutes,
      );
    }
  });
